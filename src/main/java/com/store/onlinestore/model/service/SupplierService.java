...
public class SupplierService {
    ...
    public List<Supplier> findByNameAndFamily(String name, String family) throws Exception {
        try (CrudRepository<Supplier, Long> repository = new CrudRepository<>()) {
            ...
            return repository.executeQuery("FindByNameAndFamily", params, Supplier.class);
        }
    }
    ...
    public Supplier findByMobilePhone(String mobilePhone) throws Exception {
        try (CrudRepository<Supplier, Long> repository = new CrudRepository<>()) {
            ...
            List<Supplier> result = repository.executeQuery("FindByMobilePhone", params, Supplier.class);
            if (result.isEmpty()) {
                return null;
            } else {
                return result.get(0);
    ...
    public Supplier fideByNationalCode(String nationalCode) throws Exception {
        try (CrudRepository<Supplier, Long> repository = new CrudRepository<>()) {
            ...
            List<Supplier> result = repository.executeQuery("FideByNationalCode", params, Supplier.class);
            if (result.isEmpty()) {
                return null;
            } else {
                return result.get(0);
